...
def LoRa_decode_swap(symbols):
    ...
    for ctr in range(len(symbols)):
        symbols_swp[ctr] = (((int(symbols[ctr]) & 15) << 4) | (
                (int(symbols[ctr]) & 240) >> 4))  # swap first half of 8-bit sequencne with other half
    # end for loop
...
def LoRa_decode_gray(symbols):
    symbols_gray = np.bitwise_xor(np.uint64(symbols),
                                  np.uint64(np.floor(np.right_shift(np.uint64(symbols), np.uint64(1)))))
    return symbols_gray
...
def RadioPacketComputeCrc(buffer, crcType):
    IBM = 1
    polynomial = np.int16(0xC002) if (crcType == IBM) else np.int16(0x8810)
    crc = np.uint16(0xFFFF) if (crcType == IBM) else np.uint16(0x1D0F)
...
def verifyCRC(symbols, payload_len):
    crc = symbols[int(payload_len - 4):int(payload_len-2)]
    if len(crc) == 0:
        return False
    message = symbols[0:int(payload_len - 4)]
    message = np.array(message[4:], dtype=np.int8)
    crc2 = RadioPacketComputeCrc(message, 0)

    # if (crc[0] == (crc2 & 0xff00) >> 8 and crc[1] == (crc2 & 0xff)) or (crc[0] == (crc2 & 0xff00) >> 8 or crc[1] == (crc2 & 0xff)):
    if (crc[0] == (crc2 & 0xff00) >> 8 and crc[1] == (crc2 & 0xff)):
        return True
    else:
        return False
...
## Decode Header
def lora_decoder(symbols_message, SF, CRC_check):
    rdd_hdr = 4
    ppm_hdr = SF - 2
    ...
    # symbols_pld = symbols_message[8:int(8 + (pld_length + 4 - (pld_length % 4)) * ((4.0 + CR_pld) / 4.0))]
    ...
    symbols_pld_hdr = np.hstack(
        (int(SF > 7) * symbols_hdr_shf[((len(symbols_hdr_shf) - SF + 8) - 1):len(symbols_hdr_shf)], symbols_pld_shf))
    ...
    if CRC_check:
        if verifyCRC(symbols_pld_fin, pld_length):
            return message_full
        else:
            return None
    else:
